...
float bs = 0.35; // player bullet speed
...
enum Phase { startScreen, gamePlaying, gameOver};
...
void checkButton()
{
  if (digitalRead(BUTTON_LEFT) == 0 && digitalRead(BUTTON_RIGHT) == 1)
    if (pX > 2)
      pX = pX - pXs;

  if (digitalRead(BUTTON_RIGHT) == 0 && digitalRead(BUTTON_LEFT) == 1)
    if (pX < 135 - pW - 1)
      pX = pX + pXs;
